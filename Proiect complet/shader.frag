...
vec4 getDist(vec3 poz){//dist de la o pozitie la cel mai apropiat obiect (din orice directie)
    //Aici avem obiectele noastre.
    float hPlan = 0.3*(sin(poz.x)+cos(poz.z));
    vec4 distPlan = dPlan(poz-vec3(0.0, hPlan, 0.0), vec3(0.8, 1.5, 0.0));

    //Nota muzicala 1 (un tor (gogoasa) si o capsula (cilindru rotund la capete))
    float sine = 1.5*sin(0.1*dt);
    float doiDt = 0.4*dt;
    float dtPlusPi = 0.2*dt + pi;
    float cPePatrDt = 0.25*dt;
    vec3 pozNota = vec3(sine*sin(0.2*dt), 0.0, sine*cos(0.2*dt));
...
vec4 rayMarch(vec3 pozOrig, vec3 dir){//dist. de la o pozitie la cel mai apropiat obiect in directia dir
...
float getLight(vec3 poz){
    ...
    float dist = rayMarch(poz + 2.0*DIST_MIN*norm, dirLum).w;
...
void main(){
    ...
    vec4 dist = rayMarch(pozCam, dirCam);
